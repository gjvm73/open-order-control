...
export default function Home() {
  ...
  const handleOpenCompleteChangesReport = () => {
    if (activeTab !== "active") {
      setActiveTab("active");
      window.setTimeout(() => {
        document.getElementById("relatorio-gerencial")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 0);
      return;
    }
    ...
    <div className={`min-h-screen bg-white text-zinc-950 font-sans selection:bg-red-600 selection:text-white ${isDarkMode ? "dark-mode" : ""}`}>
      ...
      <div className="border-b border-zinc-900 bg-zinc-950 text-white px-6 py-3 flex items-center justify-between no-print">
        <div className="flex flex-wrap items-center gap-4 md:gap-6 text-xs font-mono uppercase tracking-wider">
          ...
          <button type="button" onClick={handleOpenCompleteChangesReport} className="pb-1 border-b-2 border-transparent text-zinc-400 transition-all hover:border-red-600 hover:text-white">Relatório Gerencial</button>
        </div>
      ...
      <main className="print-report p-6 md:p-10 max-w-[1500px] mx-auto space-y-12">
        ...
        <section className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6">
          ...
          <div className="border border-zinc-900 overflow-hidden">
            ...
            <div className="divide-y divide-zinc-200">{stats.mostChanged.length === 0 ? <p className="p-8 text-center text-xs font-mono text-zinc-500">Ainda não há histórico suficiente.</p> : (pdfMode === "detailed" ? stats.mostChanged : stats.mostChanged.slice(0, 6)).map((item, index) => <div key={item.id} className="p-4 flex items-center gap-3"><span className="w-7 h-7 bg-zinc-950 text-white flex items-center justify-center text-xs font-bold">{String(index + 1).padStart(2, "0")}</span><div className="min-w-0 flex-1"><p className="font-bold font-mono truncate">{item.item}</p><p className="text-xs text-zinc-500 truncate">{item.customerPo || "Sem PO"} · {item.currentPrediction || "Sem previsão"}</p></div><div className="text-right"><p className="font-black text-red-600">{item.predictionChangesCount}x</p><p className="text-[10px] font-mono text-zinc-500">{formatCurrency(item.extendedPrice)}</p></div></div>)}</div>
          </div>
        </section>
        ...
        <section id="relatorio-gerencial" className="space-y-4 scroll-mt-6">
